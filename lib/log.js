        ...
        return(
            <div>
                ...
                <CommandBar className="footbar" style={{textAlign:'left'}}
                    onSelect={(cmd)=>this.context.router.transitionTo("log",cmd=='All' ? null : {level:cmd.toLowerCase()})}
                    items={["Back", "Http", "Error", "Warning", "All"]}/>
            </div>
        )
    }
}
...
class ALog extends Component{
    render(){
        ...
        return (<List.Item primaryText={`${log.level} on ${log.createdAt}`} secondaryText={log.message}/>)
    }
}
