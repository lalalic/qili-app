...
module.exports = { //for testable
    ...
    return new Promise(function (resolve, reject) {
      ...
      var getToken = function getToken() {
        if (typeof token == "string") {
          ...
          return token(props.key).then(function (_ref) {
      ...
      getToken().then(function (token) {
        dataAsBlob(data).then(function (data) {
          ...
          xhr.onreadystatechange = function () {
            if (xhr.readyState === 4) {
              if (xhr.status >= 200 && xhr.status < 300) resolve(JSON.parse(xhr.responseText).data.file_create);else reject(xhr.responseText);
            }
          };
  ...
  withFileCreate: function withFileCreate() {
