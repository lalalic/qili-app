...
export default function main(type="json", width, height){
    ...
    input.onchange=function(){
        ...
        reader.onload=function(){
			...
			case 'image':	
				...
				p.resolve({url:data, binary:toBinary(data), name})
				break
			case 'json':
				p.resolve({data:JSON.parse(data), name})
				break
			case 'jsonInJs':
				p.resolve({data: data && new Function("","return "+data)(), name})
				break
			default:
				p.resolve({data,name})
			}
