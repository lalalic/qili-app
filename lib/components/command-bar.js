var React=require('react/addons'),
    ...
    {Before:BackIcon, Home:HomeIcon, Comment:CommentIcon, Share:ShareIcon}=require('../icons/navigate'),
    enhancedBack=false;
        ...
        var {onSelect, className, ...others}=this.props,
            ...
            commands=this.props.items.map(function(command){
                ...
                if(typedOf(command, DialogCommand)){
                    var {onDismiss, label, self, icon, ...others}=command.props
                    return (
                        <Command label={label} icon={icon} key={label}
                ...
                return (
                    <Command key={command.action} onSelect={onSelect} closeDialogCommand={closeDialogCommand} {...command}/>
                )
            }.bind(this))
...
class Command extends Component{
    render(){
        var {closeDialogCommand, onSelect, action, label, icon:Icon, children}=this.props
        var icon=(Icon ? <Icon/> : <span/>);
        return (
            <div>
