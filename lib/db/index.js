...
function fixMinimongo(localDb){
    //auto add collection on remote and local
    ...
    //cache's first input params must be array
    localDb.addCollection('axx')
    let LocalDB=localDb.axx.constructor,
        _raw=LocalDB.prototype.cache;

    LocalDB.prototype.cache=function(docs,...others){
        if(!Array.isArray(docs))
            docs=[docs]
        return _raw.call(this,docs,...others)
    }
    localDb.removeCollection('axx')
}

function ajaxRequest(method, url, params, data, success, error, _appId, _sessionToken) {
    if(!appId)
...
export function init(_server,_appId, success, httpError, _loadingHandler, httpclient=ajaxRequest, options){
    makeEnvReady()
