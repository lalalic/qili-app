...
*/
var {HybridDb}=require('minimongo')
...
export default function fix(db){
    ...
    (function(){
        ...
        db.upload=HybridCollection.prototype.upload=function(){
        ...
        ;(function(find){
            ...
            HybridCollection.prototype.find=function(selector,options={}){
                var fetcher=find.call(this,selector,options).fetch,
                    interim=options.interim||this.options.interim,
                    time=0, called=false
                return {
                    fetch: function(success, error){
                        fetcher((docs)=>{
                            time++
                            if(interim && time==1 && docs.length==0){
                                //modify local Data to make always call success with remote result
